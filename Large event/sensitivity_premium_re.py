...
# Surplus modeling in time with reinsurance and without, with risk of rare large event:
...
for P in np.linspace(50,150,11):
    ...
    for n in range(trials):
        U_array, U_array_cov = U_wo(t,kappa,P,P_re,N,U_0,initial,t_M,t_mu,c_hat,frac,0.7,3*0.3/4,0.3/4)
...
fig1 = plt.figure()
plt.title('Premium dependence with p1=0.7, Pf3')
plt.errorbar(np.linspace(50,150,11),PR1[0,:],yerr=100/np.sqrt(1000),label='without coverage')
plt.errorbar(np.linspace(50,150,11),PR1[1,:],yerr=100/np.sqrt(1000),label='with coverage')
plt.xlabel('Premium')
plt.ylabel('Prob of Ruin')
plt.legend()
#plt.show()
fig1.savefig('PR_premium_re.png')
