...
# Surplus modeling in time with and without reinsurance, with the risk of large event happening:
...
def U_wo(t,kappa,P,P_re,N,U_0,initial,t_M,t_mu,c_hat,p1,p2,p3):
    ...
    if t > 0:
        ...
        for i in range(1,t+1):
            frac = np.random.normal(0.8,0.4) # fraction of people who pay premium
...
#  Approximate probability of ruin, and see which portfolio best for lower fraction of payers
# change portfolio while keeping frac at 80 % with 40% STDEV
# to test how investing is good option if there is more uncertain fraction of payers

# array to hold all different probs of ruin wrt portfolio 1:
PR1 = np.zeros((2,10))
i = 0
for p1 in np.linspace(0.1,1,10):
    ...
    for n in range(trials):
        U_array, U_array_cov = U_wo(t,kappa,P,P_re,N,U_0,initial,t_M,t_mu,c_hat,p1,p2,p3)
...
fig1 = plt.figure()
plt.title('Portfolio 1 with .4 stdev in frac')
plt.errorbar(np.linspace(0.1,1,10),PR1[0,:],yerr=100/np.sqrt(1000),label='without coverage')
plt.errorbar(np.linspace(0.1,1,10),PR1[1,:],yerr=100/np.sqrt(1000),label='with coverage')
plt.xlabel('p1, cash holding')
plt.ylabel('Prob of ruin')
plt.legend()
fig1.savefig('PR1_uncfrac_re.png')
...
fig2 = plt.figure()
plt.title('Portfolio 2 with .4 stdev in frac')
plt.errorbar(np.linspace(0.1,1,10),PR2[0,:],yerr=100/np.sqrt(1000),label='without coverage')
plt.errorbar(np.linspace(0.1,1,10),PR2[1,:],yerr=100/np.sqrt(1000),label='with coverage')
plt.xlabel('p1, cash holding')
plt.ylabel('Prob of ruin')
plt.legend()
fig2.savefig('PR2_uncfrac_re.png')
...
fig3 = plt.figure()
plt.title('Portfolio 3 with .4 stdev in frac')
plt.errorbar(np.linspace(0.1,1,10),PR3[0,:],yerr=100/np.sqrt(1000),label='without coverage')
plt.errorbar(np.linspace(0.1,1,10),PR3[1,:],yerr=100/np.sqrt(1000),label='with coverage')
plt.xlabel('p1, cash holding')
plt.ylabel('Prob of ruin')
plt.legend()
fig3.savefig('PR3_uncfrac_re.png')
