...
# change st dev of fraction of premium payers while keeping the mean constant
...
# array to hold all different probs of ruin wrt frac:
PR1 = np.zeros(len(np.linspace(0.1,0.4,15)))
i = 0
for stdev in np.linspace(0.1,0.4,15):
    ...
    for n in range(trials):
        ...
        U_array = U(t,kappa,P,N,U_0,initial,t_M,t_mu,c_hat,frac,0.9,0.75*0.1,0.25*0.1)
...
fig1 = plt.figure()
#plt.plot(np.linspace(0.1,0.4,15),PR1,label='Uncertainty in fraction of payers, with p1=0.9, Pf3')
plt.errorbar(np.linspace(0.1,0.4,15),PR1,yerr=100/np.sqrt(1000),marker='*',label='Uncertainty in fraction of payers, with p1=0.9, Pf3')
plt.xlabel('Uncertainty in fraction of payers')
plt.ylabel('Prob of Ruin')
plt.legend()
#plt.show()
fig1.savefig('PR_frac.png')
